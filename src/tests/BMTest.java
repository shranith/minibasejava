...
SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, replacealgo[i]);
    ...
    for ( index=0; status == OK && index < numPages/2; ++index ) {
      try {
        pid = SystemDefs.JavabaseBM.newPage( pg, 1 );
       // System.out.println("I'm here at new page "+pid);
      }
      catch (Exception e) {   
	status = FAIL;
  ...
  protected boolean test4 () {
	    ...
	    Page pg = new Page ();
	    PageId pid, lastPid, pgid;
	    PageId firstPid = new PageId();
	    //boolean status = OK;
	    SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, "LRUK");
	    LRUK replacer=(LRUK)(SystemDefs.JavabaseBM.getReplacer());
	    int frames[]=replacer.callframes();
		...
		int pagenumber=-1;
	        ...
	        try
	        {
	        	pagenumber=replacer.callpage(frames[index-1]);
			//pgid = replacer.getPageID(frames[index-1]);
	        	if(replacer.BACK(pagenumber,0)>space)
	        	{
	      ...
	      if (status == OK)
	      {
	    	  try
	    	  {
	    		  ...
	    		  {
	    			 pagenumber=replacer.callpage(frames[i]);
                  
			         	long currentTime = System.currentTimeMillis();
	    			 	if(replacer.HIST(pagenumber,1)>currentTime)
	    			  	{
	    				  status=FAIL;
	    			  	}
	    			  }
